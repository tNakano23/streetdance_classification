...
def parse_gm_per_classes(item, pattern=pattern_pet):
    ...
    for line in item.splitlines():
        ...
        inner_matches = re.findall(r"【(.*?):(\d+)件】", result_dict["inner"])
...
FILE_PATH = "/net/nas6/data/home/tanaka_n/dance_mm/data_result/evaluate_result/202605132157_grad_score/summary_detail.txt"
...
for content in contents:
    ...
    gm_result = parse_gm_per_classes(result_dict["gm_per_classes"])
...
move_filter = st.sidebar.multiselect("Move", sorted(df["g_m"].unique()))

csv_filter = st.sidebar.multiselect("CSV", sorted(df["csv"].unique()))

if genre_filter:
    df = df[df["genre"].isin(genre_filter)]

if move_filter:
    df = df[df["g_m"].isin(move_filter)]

if csv_filter:
    df = df[df["csv"].isin(csv_filter)]

# select_csv_mode = st.sidebar.radio("select_csv_mode", ["multiselect", "radio"])

# if select_csv_mode == "multiselect":
#     csv_filter = st.sidebar.multiselect("CSV", sorted(df["csv"].unique()))
#     df = df[df["csv"].isin(csv_filter)]
# elif select_csv_mode == "radio":
#     csv_filter = st.sidebar.radio("CSV", sorted(df["csv"].unique()))
#     df = df[df["csv"] == csv_filter]
...
st.dataframe(mistakes.head(30), use_container_width=True)
...
if isinstance(csv_filter, list):
    csv_name = ",".join(
        csv_filter_parallel_dict[x] for x in csv_filter if x in csv_filter_parallel_dict
    )

elif csv_filter in csv_filter_parallel_dict:
    csv_name = csv_filter_parallel_dict[csv_filter]

# if csv_filter in csv_filter_parallel_dict.keys():
#     csv_name = csv_filter_parallel_dict[csv_filter]
# =========================================================
# genre heatmap
# =========================================================
...
st.plotly_chart(fig_genre_heat, use_container_width=True)
...
st.plotly_chart(fig_move_heat, use_container_width=True)
